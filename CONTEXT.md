...
| ~~**B-11**~~ | ✅ **Cerrado el 31/8/2026.** Los umbrales se contrastaron contra la Tabla 1 del Decreto 151/2022 y la **calculadora oficial de ANMAT**. Cuatro coincidían; **dos estaban mal y se corrigieron**: al sodio le faltaba la condición alternativa `≥300 mg/100 g`, y el corte de calorías de bebidas era **70 cuando la norma dice 25** (70 es el valor chileno). **Los dos erraban de menos**: el usuario veía menos octógonos de los que su envase lleva. `ENGINE_VERSION` → `v2.2` para que Redis no siga sirviendo los viejos. Detalle en `nutricion/NUTRICION.md §N6` | ✅ `fitogenix-server` `seals.ts` · 415 tests |
